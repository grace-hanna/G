...
// Parse the next token, and raise an error if it's invalid
static void advance() {
...
static uint8_t argumentList() {
    ...
    consume(TOKEN_RIGHT_PAREN, "Expec ')' after arguments.");
    return argCount;
}
...
static void varDeclaration() {
    ...
    // Initialize a variable to NIL if no value is given.
    else {
        emitByte(OP_NIL);
    }
...
// Skip over tokens until we hit a statement boundary,
// where either the previous token ended a statement (;) or
// the current token starts a statement.
